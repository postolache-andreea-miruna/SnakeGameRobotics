const int totalSettingsParts = 8;
String const settingsParts[totalSettingsParts] = { "DIFFICULTY", "LCD CONTRAST", "LCD BRIGHT", "MATRIX BRIGHT", "SOUNDS", "RESET HS", "SPEED", "BACK TO MENU" };
int settingPartPosition = 0;
const int firstSettingPosition = 0;
const int lastSettingPosition = 7;
...
//speed
const byte totalSpeed = 3;
int const speedValue[totalSpeed] = { 10, 20, 30 };
int speedPosition = 0;
const byte firstSpeedPosition = 0;
const byte lastSpeedPosition = 2;
...
void settingPart() {
  ...
  else if (settingPartPosition == 6) {
    scroll();
    lcd.write(byte(3));
  }
  else if (settingPartPosition == 7) { //back to menu
    scroll();
    lcd.setCursor(0, 1);
    lcd.print("<");
    lcd.setCursor(1, 1);
...
void saveToMemory() {
  ...
  else if (settingPartPosition == 6) {  //speed
    EEPROM.update(35, speedPosition);
  } 
...
void speedsForSnake() {
  ...
  lcd.print("SPEED: ");
  lcd.setCursor(6, 0);
  lcd.print(speedValue[speedPosition]);
  if (speedPosition == 0) 
    lcd.write(byte(4));
...
void speedMenu() {
  ...
  if (joystickUp() && state2Setting == true) {
    speedPosition--;
    if (speedPosition < firstSpeedPosition) {
      speedPosition = firstSpeedPosition;
    }
    speedsForSnake();
  }
  ...
  if (joystickDown() && state2Setting == true) {
    speedPosition++;
    if (speedPosition > lastSpeedPosition) {
      speedPosition = lastSpeedPosition;
    }
    speedsForSnake();
  }
